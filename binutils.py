...
def diff_ir(out_dir, base1, base2, llvm_diff):
    ...
    with open(diff_file, "w") as diff:
        subprocess.run(["diff", "-x", "*.bc", "-q", "-r", base1, base2], stdout=diff)
...
def extract_bc(filename: str, objcopy, dis):
    ...
    subprocess.run(
        [objcopy, filename, "--dump-section", ".llvmbc={}".format(bc_filename)]
    )
...
if __name__ == "__main__":
    ...
    if not (
        check_access(llvm_dis)
        and check_access(llvm_objdump)
        and check_access(llvm_objcopy)
        and check_access(llvm_diff)
    ):
